...
## 2026-07-09 - 复购订单唯一性用 approvedOrderKey 承载

* 决策：ScoreApplication 增加 orderNo（归一化：去空白+大写）与 approvedOrderKey（唯一，格式 ruleCode:orderNo，仅审核通过时写入）。
* 原因：直接对 orderNo 加唯一约束会阻止"被拒绝后重新提交同一订单"；只在通过时写入唯一键既保证同一订单全局只通过一次，又不影响重复提交流程。
* 影响范围：复购申请创建/审核；历史无订单号数据仍按证明文本兜底查重。
* 后续注意：若已通过申请的积分被作废后需要重新给同一订单计分，需走专项加分（唯一键仍占用）。

## 2026-07-09 - 审核规则取值时点改为申请提交时点

* 决策：approveScoreApplication 按 application.submittedAt 查生效规则；积分记录 occurredAt 记为提交时点。
* 原因：与 V2.2"新规则不追溯"口径一致，避免规则中途调整导致审核结果与队长提交时看到的预期不一致。
* 影响范围：仅影响提交后规则被调整的申请。

## 2026-07-09 - 会话策略

* 决策：SESSION_SECRET 生产缺失时启动即抛错；队长会话 7 天、管理端保持 8 小时。
* 原因：消除弱密钥回退风险；队长移动端使用频繁掉线影响体验，管理端保持较短会话控制风险。

## 2026-07-09 - ScoreApplication 与 ScoreRecord 改为一对一

* 决策：ScoreRecord.applicationId 加唯一约束，Prisma 关系由一对多改为一对一（scoreRecord）。
* 原因：业务上一个申请只应生成一条积分记录，唯一约束作为并发兜底。
* 后续注意：迁移前需核对存量数据无同一 applicationId 多条记录。

# 2026-08-20 - 队长手机号登录保留后台用户名兼容

* 决策：登录输入为 11 位中国大陆手机号时按 `User.phone` 查询，其他输入按 `User.username` 查询；登录页提示队长使用手机号、后台人员仍可使用用户名。
* 原因：队长无需记用户名，同时不破坏现有管理员和职能账号的登录方式；按格式分流避免用户名与他人手机号跨字段冲突时产生不确定匹配。
* 影响范围：登录表单、登录接口、登录失败提示和审计字段。
* 后续注意：注册手机号仍必须唯一；未来支持国际手机号时需调整格式判断。

# 2026-08-20 - 老用户姓名使用独立可空字段

* 决策：在 `ScoreApplication` 新增 `repurchaseCustomerName` 可空字段；新复购申请不再要求或写入 `orderNo`，也不再依据姓名生成 `approvedOrderKey`。旧 `orderNo` 和 `approvedOrderKey` 暂保留用于历史数据兼容；“同一订单只归属 1 名队长”继续由审核人员结合证明材料人工判断。
* 原因：姓名不是唯一标识，不能沿用订单号唯一查重语义；独立字段能避免数据库命名与业务含义错位，并保留已上线历史申请数据。
* 影响范围：Prisma schema/migration、队长申请和重新提交、后台审核详情、审计日志、积分规则快照。
* 后续注意：同名老用户允许存在；审核人员需结合证明材料人工判断，不能以姓名自动拒绝重复申请，也不能把姓名当作订单号唯一键。
