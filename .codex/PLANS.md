...
- 背景：修复工作树包含两次既有评审提交及三组未提交修改；main 误跟踪本地工作树 gitlink 和数据库备份。
- 目标：完成备案、验收环境清理、拆分提交、双阶段修复分支推送、main 清理、整合验证、main 快进与 GitHub 推送。
- 非目标：不部署 CloudBase、不连接目标数据库、不执行 migration、不删除或改写分支历史。
- 当前代码理解：main 与修复分支从 `c74686f` 分叉；main 的 `4403547` 含本地路径误跟踪，修复分支的 `6e85948`/`9530325` 及未提交业务修改应保留。
- 分阶段计划：恢复与备案 → 临时环境清理 → 三个提交 → 首次推送 → main 清理 → 合并和冲突处理 → 全量验证 → 第二次推送 → main ff-only → 最终验证和推送。
- 每阶段验收标准：附件列出的状态、提交清单、测试命令和 ahead/behind 条件全部满足；任一停止条件触发则不继续。
- 当前进度：恢复检查进行中，尚未 fetch、备案或创建提交。
- 已完成：附件、规范、恢复文件和两处 Git 状态核对。
- 未完成：其余全部执行阶段。
- 风险：数据库文件在历史中可能含敏感数据，本轮仅停止继续跟踪；合并 `.codex` 必须保留双方追加历史。
- 恢复说明：读取 TASK_STATE/WORKLOG 后，从 NEXT_ACTIONS 首个未完成项继续，先确认没有新的用户或远程变更。
