...
function errorResponse(error: unknown) {
  ...
  console.error(error);
  return NextResponse.json({ success: false, error: "图片上传失败，请稍后重试" }, { status: 500 });
}

export async function POST(request: NextRequest) {
  try {
    const user = await requireRole(["LEADER"], "/api/leader/applications/upload-evidence", {
      mode: "throw",
    });
    ...
    for (const file of files) {
      ...
      const uploaded = await uploadEvidenceImageToCos({
        folder: `score-applications/${leader.id}`,
        extension,
        body: buffer,
