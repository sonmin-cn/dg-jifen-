...
function assertCanResetLocalData() {
  if (process.env.NODE_ENV === "production") {
    throw new Error("seed:test 禁止在 NODE_ENV=production 环境执行。");
  }

  if (process.env.CONFIRM_SEED_TEST !== "RESET_LOCAL_DATA") {
    throw new Error(
      'seed:test 会清空本地测试业务数据。请确认在本地开发库执行，并使用 CONFIRM_SEED_TEST=RESET_LOCAL_DATA npm run seed:test',
    );
  }
}
...
async function main() {
  ...
  console.log(`Local test seed completed, ${defaultRuleCount} default score rules ensured`);
  console.table([
